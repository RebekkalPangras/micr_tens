...
MICR_WEIGHTS_PATH = os.path.join(ROOT_DIR, "mask_rcnn_coco.h5")
...
class MicrDataset(utils.Dataset):

    def load_micr(self, dataset_dir, subset):
        """Load a subset of the IDV dataset.
        dataset_dir: Root directory of the dataset.
        subset: Subset to load: train or val
        """
        # Add classes. We have only three class to add.
        self.add_class("micr", 1, "micr")
        self.add_class("micr", 2, "accno")
        self.add_class("micr", 3, "amount")
        self.add_class("micr", 4, "bankname")
        self.add_class("micr", 5, "date")
...
def train(model):
    """Train the model."""
    # Training dataset.
    dataset_train = MicrDataset()
    dataset_train.load_micr(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = MicrDataset()
    dataset_val.load_micr(args.dataset, "val")
    dataset_val.prepare()
...
if __name__ == '__main__':
    ...
    if args.command == "train":
        ...
        class InferenceConfig(MicrConfig):
